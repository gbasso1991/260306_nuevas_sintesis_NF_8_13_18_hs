...
04 Oct 24
Actualizada constant de calibracion del paramagneto, calculada para bobina captora de N=1 espira
'''
import time
...
concentracion =(41)*1e3 #[concentracion]= g/m^3 (1 g/l == 1e3 g/m^3) (Default = 10000 g/m^3)
...
if templog:
    ...
    ax[0].scatter(temp_m, Tau,c=colors, marker='o', label=r'$\tau$',zorder=2)
    ...
    #ax[0].axhspan(tau_all.nominal_value-tau_all.std_dev,tau_all.nominal_value+tau_all.std_dev, xmin=0, xmax=1,color='tab:red',alpha=0.3,zorder=-1)

    ax[1].scatter(temp_m, SAR,c=colors,marker= 'o', label=f'',zorder=2)
    ax[1].plot(temp_m, SAR,zorder=-1)
    ax[1].axhline(SAR_all.nominal_value,0,1,c='g',ls='-.',label=f'$<$SAR$>$ = {SAR_all:.1uf} W/g')
    #ax[1].axhspan(SAR_all.nominal_value-SAR_all.std_dev,SAR_all.nominal_value+SAR_all.std_dev, xmin=0, xmax=1,color='tab:green',alpha=0.3,zorder=-1)
    ...
    ax[1].scatter(time_m, SAR,c=colors,marker= 'o', label=f'')
    ax[1].axhline(SAR_all.nominal_value,0,1,c='g',ls='-.',label=f'$<$SAR$>$ = {SAR_all:.1uf} W/g')
    ...
    for a in ax:
        # a.axvspan(tiempo _interpolado[indx_TF_interp[0][0]],tiempo_interpolado[indx_TF_interp[0][-1]],color='tab:red',alpha=0.5,label=f'T Fase: {t_tf} s',zorder=-2)
        a.legend(ncol=2)
        a.grid()
    ax[0].set_title(f'\n{nombre.strip("*")} - {frec_nombre[0]/1000:>3.0f} kHz - {round(np.mean(Campo_maximo)/1e3):>4.1f} kA/m')
    plt.suptitle(r'$\tau$ - SAR',fontsize=15)
    ...
    ax[3].scatter(temp_m, Mag_max,c=colors,marker='v',label='M$_{max}$')
    ax[3].plot(temp_m,Mag_max,zorder=-1)
    ...
    fig2, ax = plt.subplots(4, 1, figsize=(9,8), constrained_layout=True,sharex=True)
    ax[0].scatter(time_m,Coercitividad_kAm,c=colors, marker='o',label=f'H$_C$ = {Coercitividad_all:.1f} kA/m')
    ax[0].plot(time_m,Coercitividad_kAm,zorder=-1)
    ax[0].set_ylabel('Campo Coercitivo (kA/m)')

    ax[1].scatter(time_m, xi_M_0,c=colors, marker='s',label='$\chi_{M=0}$ = '+f'{xi_all:.2e}')
