...
La idea es:
    -> run routine
    -> save 2 files per second with average of 10/20 measurements
    -> stop routine
"""
from vds1022 import *
import numpy as np
...
def promediador_señales(freq=20, num_prom=20, name='xxxkHz_yyydA_100Mss_ciclo', subdir=True, num_ciclos=False,plot_range=0.04):
    ...
    for indx, frame in enumerate(owon.fetch_iter(freq=freq)):
        ...
        if (indx > 0 and indx % (num_prom - 1) == 0):
            fecha_archivo=datetime.now().strftime("%y%m%d_%H:%M:%S.%f")
            print('-'*50)
            ...
            ciclo_out.meta['comments'] = [fecha_archivo]
            ascii.write(ciclo_out, os.path.join(os.getcwd(), datename, name + str(indx // 20).zfill(4) + '.txt'),
...
owon.set_channel(channel=CH1,range='0.5v',offset=0.5,probe='10x',coupling=AC)
...
promediador_señales(freq=20,num_prom=20,
                    name='300kHz_100dA_100Mss_bobN1NF18h',
                    plot_range=0.2,num_ciclos=180)

#%%
# owon.dispose()
#%%
# promediador_señales_tiempo(freq=20,duracion=10,
#                            name='135kHz_150dA_100Mss_Ni0sR',
#                            plot_range=0.05)
...
#     x,y=frame.ch1.x()-frame.ch1.x()[0],frame.ch1.y()
#     g,h=frame.ch2.x()-frame.ch2.x()[0],frame.ch2.y()
#     iter_plotter(x,y,g,h,indx,plot_range_CH1=0.5,plot_range_CH2=20)
#
# # # #%%
# owon.dispose()
# %%
